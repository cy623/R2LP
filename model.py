...
class MLP_NORM(nn.Module):
    def __init__(self, nnodes, nfeat, nhid, nclass, dropout, alpha, beta, gamma, delta, norm_func_id, norm_layers, orders, orders_func_id, cuda):
        ...
        self.mlp = MLP(nnodes, nfeat, nhid, nclass, dropout, delta)
        self.nclass = nclass
        self.dropout = dropout
    ...
    def forward(self, x, adj, y_clean, y_unknown, args, if_lp):
        x = self.mlp(x, adj)
        h0 = x
        ...
        if if_lp:
            ...
            z, fl = self.norm_func2(x, h0, adj, y_clean, y_unknown, y_predict, args)
    ...
    def norm_func2(self, x, h0, adj, y_clean, y_unknown, y_predict, args):
        ...
        alpha = args.alpha1
        miu = (1-args.alpha1)*args.alpha2
        lamada = (1-args.alpha1)*(1-args.alpha2)*args.alpha3
        delta = (1-args.alpha1)*(1-args.alpha2)*(1-args.alpha3)
...
def new_clean(F_t, pre_select, idx_clean, idx_unknown, labels, y_clean, origi_labels):
    labels = labels.clone()
    ...
    if idx_unknown.shape[0]:
        ...
        acc = accuracy(p[idx_add], origi_labels[idx_add])
        print('select_acc:', acc)
        ...
        new_idx_unknown = torch.tensor(idx_unknown_list)
